...
class PodcastResource extends Resource implements HasShieldPermissions
{
    use FilamentContent,HasContentField;

    protected static ?string $model = Podcast::class;

    protected static ?string $navigationIcon = 'fas-podcast';


    protected static ?string $label = 'پادکست';
                ...
                Forms\Components\Grid::make(12)->schema([
                    Forms\Components\Grid::make()->schema([
                        ...
                        TiptapEditor::make('body')
                            ...
                            ->required()
                            ->columnSpanFull(),
                        Forms\Components\FileUpload::make('attachments')->label('فایل پادکست')->required()->columnSpanFull()
                    ])->columnSpan(8),
                    Forms\Components\Grid::make()->schema([
                        ...
                        Forms\Components\Section::make('سئو')->schema([
                            Forms\Components\TextInput::make('seo_title')
                                ->label('عنوان سئو')
                                ->helperText('عنوان پادکست به عنوان پیشفرض'),
                            Forms\Components\Textarea::make('meta_desc')->label('کلمات کلیدی')
                        ])->description('تنظیمات پادکست در گوگل')
                    ])->columnSpan(4),
                ])
            ]);
    }
